package com.hatoms.tms.config;

import com.hatoms.tms.security.JwtAuthFilter;
...
public class SecurityConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http, JwtUtil jwt) throws Exception {
    http
      ...
      .authorizeHttpRequests(auth -> auth
        ...
        .requestMatchers("/api/system/flags").permitAll() // Feature-Flag-Endpoint (für v0-Workspace)
      ...
      .headers(headers -> {
        ...
        headers.contentSecurityPolicy(csp -> csp.policyDirectives(String.join(" ",
                ...
                // Frontend-Entwicklung (Live Server / http-server / Vite) + Backend
                "connect-src 'self' http://localhost:8000 http://127.0.0.1:8000 http://localhost:5500 http://127.0.0.1:5500 http://localhost:5173 http://127.0.0.1:5173 http://localhost:8081;"
        )));
  ...
  CorsConfigurationSource corsConfigurationSource() {
    ...
    cfg.setAllowedOrigins(List.of(
      ...
      "http://localhost:5173", "http://127.0.0.1:5173" // Vite (v0-UI)
      // Prod: "https://deinedomain.de"
    ));
    cfg.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "OPTIONS"));
    cfg.setAllowedHeaders(List.of("Authorization", "Content-Type"));
//    cfg.setAllowCredentials(true);
    // Wir nutzen Bearer-Token, keine Cookies -> Credentials nicht nötig
